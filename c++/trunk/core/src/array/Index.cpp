...
#define INDEX_ERROR(a,b) throw cdma::Exception("Index error", a, b)
...
{
  CDMA_FUNCTION_TRACE("Index::Index");
...
{
  CDMA_FUNCTION_TRACE("Index::Index(int rank, int shape[], int start[])");
  m_rank    = rank;
  m_vCurPos.resize(m_rank);
  m_pRanges.resize(m_rank);
...
void Index::set(std::vector<int> position)
{
  ...
  {
    INDEX_ERROR("Given position doesn't match to index's rank!", "Index::set");
  }
...
void Index::setDim(int dim, int value)
{
  if( dim >= m_rank || dim < 0 )
  {
    INDEX_ERROR("Requested dimension doesn't exist!", "Index::setDim");
  }
...
{/*
  ...
  {
    INDEX_ERROR("Origin must have same length as index's rank!", "Index::setOrigin");
  }
  int i = 0;
  while( i < origin.size() )
  {
...
{/*
  ...
  {
    INDEX_ERROR("Origin must have same length as index's rank!", "Index::setShape");
  }
...
{/*
  ...
  {
    INDEX_ERROR("Origin must have same length as index's rank!", "Index::setStride");
  }
...
void Index::setIndexName(int dim, const std::string& indexName)
{
  if( dim >= m_rank || dim < 0 )
  {
    INDEX_ERROR("Requested range is out of rank!", "Index::setIndexName");
  }
...
std::string Index::getIndexName(int dim)
{
  if( dim >= m_rank || dim < 0 )
  {
    INDEX_ERROR("Requested range is out of rank!", "Index::getIndexName");
  }
...
cdma::IIndexPtr Index::reduce(int dim) throw ( cdma::Exception )
{
  if( (dim < 0) || (dim >= m_rank) )
  {
    INDEX_ERROR("Illegal reduce dim " + ('0' + dim), "Index::reduce" );
  }
  if( m_pRanges[dim].length() != 1 )
  {
    INDEX_ERROR(std::string("Illegal reduce dim " + ('0' + dim)) + " : reduced dimension must be have length=1", "Index::reduce");
  }
