...
public class DataItem implements Cloneable
{
	...
	private int[]						m_iDimS;		// DataItem (node) dimension's sizes
	private int[]						m_iDimData;		// Data slab dimension's sizes
	private int[]                       m_iStart;       // Data slab start position
	//private SoftReference<Object>		m_oData;		// Data it's an array of values belonging to a DataItem
	private Object		m_oData;		// Data it's an array of values belonging to a DataItem
	private String						m_sNodeName;	// DataItem 's node name
	...
	private PathData					m_pnPath;		// Path were the DataItem has been READ (not used when writing)
	...
	public Object getData()
	{
		...
		{
			loadData();
			result = ((SoftReference<Object>) m_oData).get();
		}
		else if( m_oData instanceof SoftReference )
		{
			if( ((SoftReference<Object>) m_oData).get() == null )
			{
				loadData();
			}
			result = ((SoftReference<Object>) m_oData).get();
	...
	public String toString()
	{
		...
		Object data = getData();
		if( m_iType == NexusFile.NX_CHAR && null != data ) {
			str.append("\n     - Value: "+ data);
		}
		return str.toString();
	}
	...
	private void loadData()
	{
		...
		catch(NexusException ne)
		{
			try
			{
				m_oData = new SoftReference<Object>(null);
				nfrFile.closeFile();
