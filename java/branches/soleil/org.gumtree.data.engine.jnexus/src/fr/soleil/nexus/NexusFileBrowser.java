...
public class NexusFileBrowser extends NexusFileInstance {
	...
	public NexusFileBrowser(String sFilePath, String sWorkDir) {
		super(sWorkDir + sFilePath);
		m_pVirtualPath = new PathNexus();
		m_pVirtualPath.setFile(sFilePath);
		m_pVirtualPath.setWorkingDir(sWorkDir);

		m_pRealPath = new PathNexus();
		m_pRealPath.setFile(sFilePath);
		m_pRealPath.setWorkingDir(sWorkDir);
	 ...
	 */
	  ...
	  * 
	  * @param iSize
	  *            new number of available slots in the node buffer
	  */
	 ...
	 /**
	  * openFile Set specified file as the current one and open it
	  */
	 @Override
	 public void openFile(String sFilePath) throws NexusException {
		 m_pRealPath.clearNodes();
		 m_pVirtualPath.clearNodes();
		 if (m_bFileChanged) {
			 initPath(sFilePath);
		 }
		 super.openFile(sFilePath);
	 }
...
/*  // old code obsolete
		 ...
		 int iDepth = pnPath.getDepth();
		 NexusNode nnCurNode;
		 int i = 0;

		 while (i < iDepth) {
			 // Determine group and class names
			 nnCurNode = pnPath.getNode(i);
			 openNode(nnCurNode);
			 i++;
		 }
*/
			 ...
			 .pushNode(new NexusNode(sItemName, sItemClass, bIsGroup));
				 ...
				 if (m_bUpdatePath)
					 m_pVirtualPath.pushNode(new NexusNode(sItemName,
							 sItemClass, bIsGroup));
				 m_pRealPath.pushNode(new NexusNode(sItemName, sItemClass,
						 bIsGroup));
		 }
	 }
	 ...
	 protected void openSubItem(int iIndex, String sNodeClass,
		 ...
		 int iCurIndex = 0; // Index of the currently examined node in list
		 String sRegExp = "^(.*)#([0-9]*$)"; // Reg exp to rename dynamically
		 // node and get them in a correct
		 // order
		 String sItemClass, sItemName; // Class and name of the currently
		 // examined group in list
		 ...
		 // Re-map entry set to have items sorted correctly: image#2 must be before image#19
	 ...
	 public void openSignalDataNode() throws NexusException {
		 ...
		 for (Iterator<Entry<String, String> > iter = mNodeMap.entrySet().iterator(); iter.hasNext(); ) {
			 entry = iter.next();
	 ...
	 /**
	  * getNode Create a new NexusNode by scanning child of the current path according to given node name
	  */
	 public NexusNode getNode(String sNodeName) throws NexusException {
		 ...
		 .hasNext();) {
			 ...
			 if ( sItemName.equals(sCurFullName) ||
				 ...
				 return new NexusNode(sCurName, sCurClass, !(sCurClass
						 .equals("SDS") || sCurClass.equals("NXtechnical_data")));
			 }
		 }
			 ...
			 try { // don't succeed, so we try replacing "/" by "__"
				 ...
				 try { // don't succeed: trying adding "#1" at end of name
					 ...
					 try { // don't succeed: trying with "__#1" at end of name
						 ...
						 // don't succeed, no more good idea... we send an Exception
	 ...
	 public ArrayList<NexusNode> listChildren() throws NexusException {
		 ...
		 for (Iterator<Entry<String, String> > iter = hmNodeMap.entrySet().iterator(); iter.hasNext(); ) {
			 ...
			 sNodeClass = entry.getValue();
			 boolean bRealGroup = !(
		 								sNodeClass.equals("SDS") || 
		 								sNodeClass.equals("NXtechnical_data")
		 		 				  );
	 ...
	 private void initPath(String sFilePath) {
		 ...
		 m_pVirtualPath.setFile(sFilePath.replaceFirst("(.*"
				 + NexusFileInstance.PATH_SEPARATOR + ")?([^"
				 + NexusFileInstance.PATH_SEPARATOR + "]+)", "$2"));
		 m_pVirtualPath.setWorkingDir(sFilePath.replaceFirst("(.*"
				 + NexusFileInstance.PATH_SEPARATOR + ")?([^"
				 + NexusFileInstance.PATH_SEPARATOR + "]+)", "$1"));

		 m_pRealPath = new PathNexus();
		 m_pRealPath.setFile(sFilePath.replaceFirst("(.*"
				 + NexusFileInstance.PATH_SEPARATOR + ")?([^"
				 + NexusFileInstance.PATH_SEPARATOR + "]+)", "$2"));
		 m_pRealPath.setWorkingDir(sFilePath.replaceFirst("(.*"
				 + NexusFileInstance.PATH_SEPARATOR + ")?([^"
				 + NexusFileInstance.PATH_SEPARATOR + "]+)", "$1"));
	 ...
	 protected Object getAttribute(String sAttrName) throws NexusException {
		 ...
		 if (iAttrInf[1] == NexusFile.NX_CHAR) {
			 ...
			 oAttrVal = ((String) oAttrVal).substring(0, ((String) oAttrVal)
					 .length() - 1);
		 }
		 return oAttrVal;
	 }
