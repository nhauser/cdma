...
public class PathNexus implements Cloneable {
	...
	/**
	 * setWorkingDir
	 * Set the working directory. It's a reference path for the application, all links to
	 * reach a file will be depending of this path.
	 * @param sWorkDir path in string to set the working directory
	 */
	protected void setWorkingDir(String sWorkDir)									{ m_sWorkingDir = sWorkDir; }
	...
	{
		String sFilePath = "";
		if( m_sWorkingDir != null )
			sFilePath += m_sWorkingDir;
		if( m_sFilePath != null )
			sFilePath += m_sFilePath;

		return sFilePath.equals("") ? null : sFilePath;
	}
	 ...
	 */
	public String getWorkingDir()	{ return m_sWorkingDir; }
	...
	public PathGroup getParentPath()
	{
		...
		pgRes.setWorkingDir(pnBuf.m_sWorkingDir);
	...
	public String toString(boolean showFile)
	{
		if( showFile && m_sFilePath != null && !"".equals(m_sFilePath.trim())  )
			return m_sWorkingDir + m_sFilePath + ";" + getValue();
		else
			return getValue();
	}
	...
	{
		String sLocalPath = "";
		File fTarget = new File(sFilePath);
		File fParent = new File(m_sWorkingDir + m_sFilePath);

		// Check both files exist
		if( ! fTarget.exists() )
			throw new NexusException("File ("+ fTarget.getName() +") to locate doesn't exist at given path!");

		if( ! fParent.exists() )
			throw new NexusException("File ("+ fParent.getName() +") to edit doesn't exist at given path!");

		// Determinate shortest path between the two file
		sLocalPath = determinePath(fParent, fTarget);
	...
	public PathNexus clone()
	{
		...
		paPath.m_sWorkingDir = m_sWorkingDir;
		return paPath;
	}
