...
public class TestData {
	public static void testData(ILogicalGroup root) throws IOException, ShapeNotMatchException, InvalidRangeException {
		//testCopyVsCreate(root);
		testIterationOverRegion(root);
	}
	
    public static void testCopyVsCreate(ILogicalGroup root) throws IOException, ShapeNotMatchException, InvalidRangeException {
		System.out.println("------------------------------------------------------------------");
		System.out.println("------------------------------------------------------------------");
    	System.out.println("Displaying images: ");
    	IKey key = Factory.getFactory(root.getFactoryName()).createKey("images");
        ...
        if( data != null ) {
			...
			if( data != null && shape.length > 2) {
				try {
	                ...
	                IArray arr = Factory.getFactory(root.getFactoryName()).createArray(a.getElementType(), a.getShape(), oo);
	                ...
	                int l1 = buf.toString().length()  < 1000 ? buf.toString().length()  : 1000;
	                int l2 = buf2.toString().length() < 1000 ? buf2.toString().length() : 1000;
	                System.out.println(buf.toString().substring(0, l1));
	                System.out.println(buf2.toString().substring(0, l2));
	                System.out.println("Getting a copy of the region and create a new IArray with following shapes:");
	                System.out.println("Region: " + a.getIndex());
	                System.out.println("Copy: " + arr.getIndex());
	                if( buf.toString().equals(buf2.toString()) ) {
	                	System.out.println("Both arrays have equivalent contents ");
	                }
	                else {
	                	throw new IOException("Copying and creating a new array from a region of another array isn't equivalent!\nContents are different!!!");
	                }
				} catch (IOException e) {
					e.printStackTrace();
    ...
    public static void testIterationOverRegion(ILogicalGroup root) throws IOException, InvalidRangeException {
