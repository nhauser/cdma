...
public final class VcArrayIterator implements IArrayIterator {
	// / Members
	private final IArray mArray;
	private IIndex mIndex;
	private Object mCurrent;
	private final boolean mAccess; // Indicates that this can access the storage

	// memory or not

	public VcArrayIterator(VcArray array) {
		mArray = array;
		// [ANSTO][Tony][2011-08-31] Should mAccess set to true for
		// NxsArrayInterface??
		// If m_access is set to false, next() does not work.
		// [SOLEIL][Clement][2011-11-22] Yes it should. It indicates that the
		// iterator shouldn't access memory. In case of hudge matrix the next()
		// will update m_current (i.e. value), but the underlying NeXus engine
		// will automatically load the part corresponding to the view defined by
		// this iterator, which can lead to java heap space memory exception
		// (see NxsArray : private Object getData() )
		mAccess = true;
		try {
	...
	public VcArrayIterator(IArray array, IIndex index) {
		this(array, index, true);
	}

	public VcArrayIterator(IArray array, IIndex index, boolean accessData) {
