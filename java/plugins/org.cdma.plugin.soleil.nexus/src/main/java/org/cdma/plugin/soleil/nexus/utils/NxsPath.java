...
public class NxsPath extends HdfPath {
    ...
    public NxsNode[] getNodes() {
        NxsNode[] result = new NxsNode[nodes.size()];
        int i = 0;
        for (INode hdfNode : nodes) {
            result[i++] = new NxsNode(hdfNode.getName(), hdfNode.getAttribute());
        }
        return result;
    ...
    public void addNode(final INode nodeToAdd) {
        nodes.add(nodeToAdd);
    }
