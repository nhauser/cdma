...
public class DefaultCompositeIndex implements IIndex, Cloneable {

    private final DefaultIndex mIndexArrayData; // View describing the matrix of arrays
    private final DefaultIndex mIndexStorage;      // View describing each array storage
     ...
     */
    public DefaultCompositeIndex(String factoryName, int matrixRank, int[] shape, int[] start,
            int[] length) {
        ...
        mIndexStorage = new DefaultIndex(
                factoryName,
    ...
    public DefaultCompositeIndex(String factoryName, int[] shape, int[] start, int[] length) {
        this(factoryName, 0, shape, start, length);
    }

    public DefaultCompositeIndex(DefaultCompositeIndex index) {
        mIndexArrayData = (DefaultIndex) index.mIndexArrayData.clone();
        mIndexStorage = (DefaultIndex) index.mIndexStorage.clone();
    }

    public DefaultCompositeIndex(String factoryName, int[] storage) {
        mIndexArrayData = new DefaultIndex(factoryName, new int[] {});
        mIndexStorage = new DefaultIndex(factoryName, storage.clone());
    }

    public DefaultCompositeIndex(String factoryName, int[] matrix, int[] storage) {
        mIndexArrayData = new DefaultIndex(factoryName, matrix.clone());
        mIndexStorage = new DefaultIndex(factoryName, storage.clone());
    }

    public DefaultCompositeIndex(String factoryName, int matrixRank, IIndex index) {
        this(factoryName, matrixRank, index.getShape(), index.getOrigin(), index.getShape());
    ...
    @Override
    public IIndex clone() {
        return new DefaultCompositeIndex(this);
    }

    @Override
    public String getFactoryName() {
        return mIndexArrayData.getFactoryName();
    }
    ...
    public DefaultIndex getIndexStorage() {
