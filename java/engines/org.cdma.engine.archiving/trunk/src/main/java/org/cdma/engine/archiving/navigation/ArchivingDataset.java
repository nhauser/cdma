...
public class ArchivingDataset implements IDataset {
     ...
     */
    public ArchivingDataset(String factory) {
        mRootGroup = null;
        mIsOpen = false;
        mTitle = "";
        mFactory = factory;
    ...
    public IGroup getRootGroup() {
        if ((mRootGroup == null) && isOpen()) {
            ...
            if (ArchivingQueries.checkDatabaseConformity(attribute)) {
                mRootGroup = group;
                mRootGroup.addOneAttribute(new ArchivingAttribute(mFactory, Constants.DATE_FORMAT,
                        Constants.ISO_DATE_PATTERN));
            }
        }
        return mRootGroup;
    }
    ...
    public void open() throws IOException {
        synchronized (this) {
            ...
            if (mSqlDataset == null) {
                mSqlDataset = new SqlDataset(mFactory, mHost, getUser(), getPassword(), getDriver(), getDbName(),
                        getSchema(), isRac());
            }
