...
public class ArchivingGroup implements IGroup {
    ...
    private final ArchivingDataset mDataset; // handler on the dataset
    ...
    private final String mFactory; // Plug-in factory name
    ...
    protected ArchivingGroup(final String factory, final ArchivingDataset dataset) {
        this(factory, dataset, null, null);
    }

    public ArchivingGroup(final String factory, final ArchivingDataset dataset, final ArchivingGroup parent,
            final String name) {
        mFactory = factory;
    ...
    public IGroup getGroup(final String shortName) {
        ...
        if (shortName != null) {
            for (IContainer container : mChildren) {
                if (container.getModelType().equals(ModelType.Group) && container.getShortName().equals(shortName)) {
    ...
    @Override
    public IGroup getGroupWithAttribute(final String name, final String value) {
        initChildren();
        IGroup result = null;
        if (name != null) {
            for (IContainer container : mChildren) {
    ...
    @Override
    public boolean removeDataItem(final String shortName) {
    ...
    public void setSamplingPeriod(final SamplingPeriod period) {
    ...
    // Private methods
    // ------------------------------------------------------------------------
    private boolean removeContainer(final String shortName) {
        boolean result = false;
        if (shortName != null) {
            for (IContainer container : mChildren) {
                if (shortName.equals(container.getShortName())) {
    ...
    private void initProperties() {
        ...
        if (getArchivedAttribute().getPath().isFullyQualified() && !mInitialized) {
            ...
            try {
                ...
                ArchivingAttribute startArchiving = new ArchivingAttribute(mFactory, Constants.ORIGIN_DATE,
                ...
                CDMAExceptionManager.notifyHandler(this, new CDMAException(
                        "Unable to initialize the attribute properties !" + e.getMessage()));
            }
        }
    }
}
