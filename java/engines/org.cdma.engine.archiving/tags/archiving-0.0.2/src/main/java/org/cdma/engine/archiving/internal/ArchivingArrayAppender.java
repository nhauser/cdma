...
public class ArchivingArrayAppender extends ArrayDefaultAppender implements ISqlArrayAppender {
	private Attribute attribute; // Archiving attribute
	private Map<Integer, Integer> lengths;
	private Map<Integer, Boolean> converts;
	private Map<Integer, Class<?>> clazzz;
	private Map<Integer, StringArrayConverter> converters;
	...
	public DataArray<?> allocate(ResultSet set, int column, int nbRows) throws SQLException {
		...
		if( current != null && !current.equals(expected) && !expected.equals(String.class) ) {
			...
			case DOUBLE: {
					// Allocate data
					array = DataArray.allocate( (double[]) new double[length], rows);
				...
				case FLOAT: {
					// Allocate data
					array = DataArray.allocate( (float[]) new float[length], rows);
				...
				case LONG: {
					// Allocate data
					array = DataArray.allocate( (long[]) new long[length], rows);
				...
				case SHORT: {
					// Allocate data
					array = DataArray.allocate( (short[]) new short[length], rows);
