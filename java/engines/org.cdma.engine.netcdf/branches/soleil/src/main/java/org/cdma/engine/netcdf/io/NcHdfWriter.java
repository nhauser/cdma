 ...
 ******************************************************************************/
package org.cdma.engine.netcdf.io;
...
//					System.out.println(object);
	 ...
	 * (non-Javadoc)
	 * 
	 * @see org.cdma.io.IWriter#isOpen()
	 */
	 ...
	 * (non-Javadoc)
	 * 
	 * @see org.cdma.io.IWriter#writeToRoot(org.cdma.IGroup)
	 */
	 ...
	 *             failed to write
	 */
	protected void writeDataItem(final Group parent, final IDataItem dataItem, final boolean force) throws WriterException {
		if (parent == null) {
			return;
		}
		if (isDataItemExist(parent, dataItem.getShortName())) {
			if (force) {
				...
				throw new WriterException("the data item with the same "
	...
	protected Dataset writeHdfDataset(final Group parent,
		...
		try {
		    ...
		    Factory.getLogger().log( Level.SEVERE, e1.getMessage() );
	...
	protected void writeAttribute(final HObject parent,
		...
		if (isAttributeExist(parent, attribute.getName())) {
			if (force) {
				...
				throw new WriterException("an attribute already exis : "
						+ attribute.getName());
			}
		}
