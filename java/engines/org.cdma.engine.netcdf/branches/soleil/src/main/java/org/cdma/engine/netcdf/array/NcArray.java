...
import org.cdma.engine.netcdf.math.NcArrayMath;
import org.cdma.engine.netcdf.utils.NcArrayUtils;
import org.cdma.engine.netcdf.utils.Register;
...
public class NcArray implements IArray {
	 ...
	 *            Netcdf Array object
	 */
	public NcArray(final ucar.ma2.Array netcdfArray, String factoryName) {
	    this.factoryName = factoryName;
		this.array = netcdfArray;
		registerId = Register.getInstance().getArrayRegisterId(this);
		arrayMath = new NcArrayMath(this, Factory.getFactory( this.factoryName ));
		// if (registerId != 0)
	...
	@Override
	public NcArray copy() {
		return new NcArray(getArray().copy(), this.factoryName);
	}
	...
	public IIndex getIndex() {
		return new NcIndex(getArray().getIndex(), factoryName);
	}
	...
	public IArray slice(final int dim, final int value) {
		return new NcArray(getArray().slice(dim, value), this.factoryName);
	}
	...
	public IArrayIterator getIterator() {
		return new NcArrayIterator(getArray().getIndexIterator(), this.factoryName);
	}
	...
	public IArrayIterator getRegionIterator(final int[] reference,
		...
		try {
			...
			iterator = new NcArrayIterator(getArray().getRangeIterator(
					section.getRanges()), this.factoryName);
	...
	public final ISliceIterator getSliceIterator(final int rank)
		...
		return new NcSliceIterator(this, rank, this.factoryName);
	}
