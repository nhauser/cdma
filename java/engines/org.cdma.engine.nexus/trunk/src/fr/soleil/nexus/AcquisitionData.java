...
public class AcquisitionData {
    ...
    public AcquisitionData() {
        m_nfwFile = new NexusFileWriter();
        initGroupListeners();
    }

    public AcquisitionData(int iBufferSize) {
        m_nfwFile = new NexusFileWriter();
        m_nfwFile.setBufferSize(iBufferSize);
        initGroupListeners();
    }

    public AcquisitionData(String sFilePath) {
        m_nfwFile = new NexusFileWriter(sFilePath);
        initGroupListeners();
    }

    public AcquisitionData(String sFilePath, int iBufferSize) {
        m_nfwFile = new NexusFileWriter(sFilePath);
        m_nfwFile.setBufferSize(iBufferSize);
        initGroupListeners();
    }
    ...
    // / Accessors
    protected NexusFileWriter getNexusFileWriter() {
        return m_nfwFile;
    }

    /**
     * GetFile/SetFile Getter and setter of the targeted Nexus file path
     */
    public String getFile() {
        return m_nfwFile.getFilePath();
    }

    public void setFile(String sFilePath) {
        m_nfwFile.setFile(sFilePath);
    }

    /**
     * finalize Clean memory
     */
    @Override
    ...
    // ---------------------------------------------------------
    /**
     * setFlagSingleRaw set the multidimensional array mechanism to be active or
     * inactive
     * 
     * @param bSingleRaw true if we want to manage single raw arrays
     * @note by default the mechanism is on mono-dimensional arrays
     */
    ...
    public boolean getCompressedData() {
        return m_nfwFile.getCompressedData();
    }

    public void setCompressedData(boolean bComp) {
        m_nfwFile.setCompressedData(bComp);
    }
    ...
    // ---------------------------------------------------------
    /**
     * existPath return a boolean having true as value if the given path exists,
     * else return false
     * 
    ...
    /**
     * listNodes return a NexusNode array containing all nodes below the
     * specified Nexus Path
     * 
     * @param pnPath
     *            NexusPath to list
     */
    ...
    /**
     * getAcquiList return an array string containing all Acquisition's name as
     * direct children of a NeXus file
     */
    ...
    /**
     * getInstrumentList return an array string containing all instrument
     * belonging to an Acquisition (i.e.: NXpositionner, NXinsertion_device...)
     * 
     * @param sAcquiName name of the acquisition which belongs instruments
     * @note instruments aren't direct descendant of an Acquisition, they are
     *       children of an NXinstrument node
     */
    ...
    /**
     * getInstrumentList return an array string containing all instrument
     * belonging to an Acquisition, having a particular type (i.e.:
     * NXpositionner or NXinsertion_device...)
     * 
     * @param sAcquiName name of the acquisition which belongs instruments
     * @note instruments aren't direct descendant of an Acquisition, they are
     *       children of an NXinstrument node
    ...
    /**
     * getImageCount Count images belonging to an Acquisition
     * 
    ...
    /**
     * getImagePath Returns the path of the image belonging to the Acquisition
     * 
    ...
    // ---------------------------------------------------------
    /**
     * Return a specified DataItem belonging to an instrument in an acquisition.
    ...
    /**
     * GetData2D Read a 2D data from a NexusFile.
     * 
     ...
     */
    public DataItem getData2D(int iImageIndex) throws NexusException {
        return getData2D(iImageIndex, 0);
    }
    ...
    /**
     * GetInstrumentData Return an array of DataItem filled by all data stored
     * in the targeted instrument
     * 
    ...
    // ---------------------------------------------------------
    /**
     * ReadData Read data to the given path
     * 
    ...
    /**
     * readData Read all DataItems that are descendants (direct and indirect
     * children) of the given group
     * 
    ...
    /**
     * assureFileExistence Ensure user that the targeted file exists
     */
    ...
    /**
     * WriteData Write data to the given path
     * 
    ...
    /**
     * writeData Write a single data into the target path. Following methods are
     * overload of WriteData for all primitive types.
     * 
    ...
    /**
     * writeData Write each data contained in the array into its own path. The
     * targeted file is the current one of AcquisitionData.
     * 
    ...
    /**
     * writeAttr Write an attribute on the node pointed by path.
     * 
    ...
    /**
     * writeDeepCopy Copy the sibling node and all its descendants into the
     * given targeted group. All attributes of each node will be copied.
     * 
    ...
    /**
     * writeLink Create a link positioned at pgDestPath in the current file that
     * targets pgdestPath.
     * 
    ...
    public enum Instrument {
        ...
        private Instrument(String sName) {
            m_sName = sName;
        }
    ...
    // ---------------------------------------------------------
    /**
     * prepareWriteDataItem Fill the end of a NX_CHAR DataItem with
     * END_STRING_CHAR to ensure it have a length MAX_LEN_CHAR of characters. If
     * this length is not forced, data could not be update after its creation
     * (due to compatibility check). Indeed for NX_CHAR DataItem, the dimension
     * size returned is the exactly the string's length, not MAX_LEN_CHAR
     * DataItem's Initialization.
     * 
     * @param dsData
     *            DataItem to be stored
    ...
    protected List<NexusNode> listNode(String sClassName, boolean bEqualityTest) throws NexusException {
        String[] sClassNames = { sClassName };
    ...
    protected List<NexusNode> listNode(String[] sClassNames, boolean bEqualityTest) throws NexusException {
